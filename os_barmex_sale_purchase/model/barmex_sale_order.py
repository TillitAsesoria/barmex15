...
class SaleOrder(models.Model):
    ...
    previous_state = fields.Char(
        ...
        readonly=True)
    lco_sale_participants_ids = fields.Many2many(
        'crm.lead',
        string='Participants')
    lco_property_product_pricelist_id = fields.Many2one(
        'product.pricelist',
        store=True,
        copied=True,
    ...
    def get_credit_message(self):
        ...
        if not self.partner_id.exceed_limit:
            if self.partner_id.sale_order_exceeded:
                ...
                msg = _(
                    "Credit limit will be exceeded with this order.\nThis order will be locked.")
        ...
        for record in self.order_line:
            if record.lco_prod_prospecto:
                raise ValidationError(
                    _("This operation is not allowed with prospect products"))
    ...
    def _prepare_invoice(self):
        ...
        if self.partner_id.lco_sale_zone.journal_id:
            invoice_vals[
                'journal_id'] = self.partner_id.lco_sale_zone.journal_id.id
        else:
            raise ValidationError(
                'Libro contable no esta definido en Zona de ventas')
    ...
    def onchange_pricelist_id(self):
        for record in self:
            if record.pricelist_id:
                if record.currency_id.id != \
                        record.lco_property_product_pricelist_id.currency_id.id:
