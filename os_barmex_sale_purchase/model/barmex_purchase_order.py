...
class PurchaseOrder(models.Model):
    ...
    def validate_orderpoint(self):
        ...
        for line in self.order_line:
            ...
            if product.__class__._validate_offer(self, product,
                                                 line.location_dest.id) == 1:
                msj = _(
                    "Article {} is marked as offer. To continue this order has to be approved.").format(
                    line.product_id.name)
                self.state = 'offer'
                self.offer = True

            elif product.__class__._validate_offer(self, product,
                                                   line.location_dest.id) == 2:
                msj = _(
                    "Article {} is on a category marked as approbation. To continue this order has to be approved.").format(
                    line.product_id.name)
                self.state = 'pending'

        info = self.env['barmex.dialog.box'].display_dialog(title, msj, None,
                                                            self.id)
    ...
    def validate_inventary(self):
        ...
        for line in self.order_line:
            ...
            if line.product_id.qty_available_barmex > 0:
                for stock in line.product_id.stock_quant_ids:
                    if not stock.owner_id and stock.location_id.usage == 'internal':
                        ...
                        if stock.quantity > 0:
                            ...
                            msg = _("There are {} {} of {} in {}.").format(
                                on_hand, line.product_uom.name,
                                line.product_id.name,
                ...
                msg = _("Non inventary existent of: {}").format(
                    line.product_id.name)
                lines.append((0, 0, {'name': msg}))

        info = self.env['barmex.dialog.box'].display_dialog(title, message,
                                                            lines, self.id)
    ...
    def _amount_to_words(self):
        ...
        words = currency.with_context(lang='es_ES').amount_to_text(
            amount_i).upper()
    ...
    def purchase_amount_to_text(self):
        ...
        words = self.currency_id.with_context(
            lang=self.partner_id.lang or 'es_MX').amount_to_text(
            amount_i).upper()
        invoice_words = '%(words)s %(amount_d)02d/100 %(curr_t)s' % dict(
