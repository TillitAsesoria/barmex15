...
class StockMove(models.Model):
    ...
    foreign_trade_id = fields.Many2one(
        'barmex.foreign.trade',
        states={'done': [('readonly', True)]})
    foreign_trade_ids = fields.One2many(
        'barmex.foreign.trade',
        'move_id',
        string='Petitions')
    tax_id = fields.Char(
    ...
    def _foreign_trade(self):
        for record in self.move_line_ids:
            if not record.product_id.categ_id.removal_strategy_id.method in \
                   ('fifo', 'lifo'):
                raise Warning(_(
                    'Petition is unavailable for this transaction. '
                    'All products Removal Strategy has to be FIFO or LIFO.'))
            ...
            if msg:
                return (
                    self.env['barmex.alert'].create({
                        ...
                        'origin_reference': '{},{}'.format('stock.picking',
                                                           self.id),
    ...
    def get_credit_message(self):
        ...
        if self.state != 'lock':
            if self.partner_id.sale_order_exceeded and self.partner_id.sale_order_include:
                ret = _(
                    'Has one or more credit limits exceeded.\nThis delivery will be locked')
    ...
    def print_delivery_detail(self):
        return self.env.ref(
            'barmex.barmex_delivery_report_detailed').report_action(self)

    def stock_account(self):
        if self.picking_type_id.code == 'incoming':

            source = self.location_dest_id.complete_name.split('/')
            source_acc = self.env['stock.warehouse'].search(
                [('code', '=', source[0])]).stock_account
        ...
        if self.picking_type_id.code == 'internal':
            ...
            dest_acc = self.env['stock.warehouse'].search(
                [('code', '=', dest[0])]).stock_account
            accounting = []
