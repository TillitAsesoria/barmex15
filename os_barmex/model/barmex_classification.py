...
class Classification(models.Model):
    ...
    _sql_constraints = [
        ('unique_classification_configuration', 'unique(classification)',
    ...
    def one_default(self):
        count = 0
        classifications = self.env['barmex.classification'].search([
            ('default', '=', True)
        ])
        ...
        if count > 1:
            raise ValidationError(
                _('Only one classification can be marked as default'))
