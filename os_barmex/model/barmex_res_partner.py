...
class ResPartner(models.Model):
    ...
    _sql_constraints = [
        ('unique_partner_barmex_id_cust', 'unique(barmex_id_cust)',
    ...
    tax_id = fields.Char(
        string='Foreign vendor Tax ID',
        copy=False)
    credit_days = fields.Integer(string='Credit days')

    # product_reference_id = fields.One2many(
    #     'barmex.customer.codes',
    #     'partner_id')
    # account_moves_ids = fields.One2many(
    #     'account.move',
    #     'movement_id')
    # ref_payment_ids = fields.One2many(
    #     'account.payment',
    #     'payment_partner_id',
    #     string="Factoring payments")
    invoice_files = fields.One2many(
        'barmex.invoice',
        'invoice_id',
        string='Files')
    property_payment_term_id = fields.Many2one(
        ...
        help="This payment term will be used instead of the default one'"
             "for sales orders and customer invoices",
        required=True)
    name = fields.Char(
        index=True,
        required=True)
    barmex_id_cust = fields.Char(
        string='Customer ID number',
        size=6)
    barmex_id_vend = fields.Char(
        string='Vendor ID number',
        size=6)
    lco_partner_modify_salesperson = fields.Boolean(
        string='Modify salesperson',
        track_visibility=True)
    lco_property_product_pricelist = fields.Many2one(
        'product.pricelist',
        on_delete='set null',
        string='Tarifa')
    lco_property_product_pricelist_usd = fields.Many2one(
        'product.pricelist',
        on_delete='set null',
        string='Tarifa USD')
    lco_sale_zone = fields.Many2one(
        'barmex.sale.zone',
        string='Sale zone',
        stored=True,
        track_visibility=True,
        check_company=True)
    lco_fact_global = fields.Boolean(
        string='Global invoicing',
        default=False)
    lco_customer_type = fields.Many2one(
        'barmex.partner.type',
    ...
    l10n_mx_edi_usage = fields.Selection([
        ...
        help=
        'Used in CFDI 3.3 to express the key to the usage that will '
    ...
    partially_payed_invoice_include = fields.Boolean(
        string='Include Partially Payed Invoice')
    open_account_include = fields.Boolean(string='Include Open Account Moves')
    partially_open_account_include = fields.Boolean(
        string='Include Partially Open Account Moves')
    # Risk amounts
    sale_order_risk = fields.Monetary(
        string='Sale Orders Included',
        compute='_get_amounts',
        currency_field='credit_currency')
    draft_invoice_risk = fields.Monetary(
        string='Draft Invoices Included',
        compute='_get_amounts',
        currency_field='credit_currency')
    open_invoice_risk = fields.Monetary(
        string='Invoices Without Payment Included',
        compute='_get_amounts',
        currency_field='credit_currency')
    partially_payed_invoice_risk = fields.Monetary(
        string='Partially Payed Invoices Included',
        compute='_get_amounts',
        currency_field='credit_currency')
    open_account_risk = fields.Monetary(
        string='Open Account Moves Included',
        compute='_get_amounts',
        currency_field='credit_currency')
    partially_open_account_risk = fields.Monetary(
        string='Partially Open Account Moves Included',
        compute='_get_amounts',
        currency_field='credit_currency')
    total_risk = fields.Monetary(
        string='Total Risk',
        compute='_total_risk',
        currency_field='credit_currency')
    # Credit limit
    sale_order_amount = fields.Monetary(
        ...
        currency_field='credit_currency')
    draft_invoice_amount = fields.Monetary(
        string='Draft Invoices Amount',
        currency_field='credit_currency')
    open_invoice_amount = fields.Monetary(
        string='Open Invoices Amount',
        currency_field='credit_currency')
    partially_payed_invoice_amount = fields.Monetary(
        string='Partially Payed Invoice Amount',
        currency_field='credit_currency')
    open_account_amount = fields.Monetary(
        string='Open Account Moves Amount',
        currency_field='credit_currency')
    partially_open_account_amount = fields.Monetary(
        string='Partially Open Account Moves Amount',
        currency_field='credit_currency')
    general_limit = fields.Monetary(
        string='General Credit',
        currency_field='credit_currency')
    # Limit exceeded
    sale_order_exceeded = fields.Boolean(
        string='Sale Order Limit Exceeded',
        readonly=True,
        compute='_credit_limit_exceeded')
    draft_invoice_exceeded = fields.Boolean(
        string='Draft Invoice Limit Exceeded',
        readonly=True,
        compute='_credit_limit_exceeded')
    open_invoice_exceeded = fields.Boolean(
        string='Open Invoice Limit Exceeded',
        readonly=True,
        compute='_credit_limit_exceeded')
    partially_payed_invoice_exceeded = fields.Boolean(
        string='Partially Payed Limit Exceeded',
        readonly=True,
        compute='_credit_limit_exceeded')
    open_account_exceeded = fields.Boolean(
        string='Open Account Moves Exceeded',
        compute='_credit_limit_exceeded',
        readonly=True)
    partially_open_account_exceeded = fields.Boolean(
        string='Partially Open Account Moves Exceeded',
        compute='_credit_limit_exceeded',
        readonly=True)
    credit_exceeded = fields.Boolean(
        string='General Limit Exceeded',
        readonly=True,
        compute='_credit_limit_exceeded')
    ...
    due_invoice_lock = fields.Boolean(
        string='Lock By Due Invoice',
        default=True)
    exceed_limit = fields.Boolean(
        string='Exceed Credit Limit',
        default=False)
    sale_order_available = fields.Monetary(
        string='Sale Order Credit Available',
        currency_field='credit_currency',
        readonly=True,
    ...
    credit_available = fields.Monetary(
        ...
        readonly=True,
        compute='_credit_available')
    locked_by_due = fields.Boolean(
        string='Credit Lock By Due Invoice',
        readonly=True,
        compute='_due_invoice')
    collector_ids = fields.One2many(
        'barmex.collector',
        'partner_id',
        string='Collectors')
    cobrador_employee_ids = fields.Many2one(
        'hr.employee', string='Cobrador')
    corporate_res_partner = fields.Many2one(
        'barmex.corporate', 'Corporativo')
    lco_property_product_pricelist_extra = fields.Many2one(
        'product.pricelist',
        on_delete='set null',
        string='Tarifa extra')
    ...
    purchase = fields.Boolean(
        compute='_check_group',
        readonly=True)
    invoicing_currency = fields.Boolean(
        string="Invoicing in National Currency")
    ...
    def _due_invoice(self):
        for record in self:
            invoices = self.env['account.move'].search(
                [('partner_id', '=', record.id), ('state', '=', 'posted'),
    ...
    def _get_amounts(self):
        for record in self:
            ...
            sales = self.env['sale.order'].search(
                [('partner_id', '=', record.id), ('state', '=', 'sale')])
            ...
            for order in sales:
                sale += order.currency_id._convert(
                    order.amount_total,
                    record.credit_currency,
                    self.env.company,
                    order.date_order)
            ...
            for invoice in invoices:
                ...
                if invoice.state == 'draft' or invoice.state == 'lock':
                    draft += invoice.currency_id._convert(
                    ...
                    partial += invoice.currency_id._convert(
                        invoice.amount_residual, record.credit_currency,
                        self.env.company, invoice_date)

                elif invoice.state == 'posted' and invoice.amount_total == invoice.amount_residual:
                    open += invoice.currency_id._convert(
                        invoice.amount_total,
                        record.credit_currency,
                        self.env.company,
                        invoice_date)
            ...
            for move in moves:
                ...
                if not move.matched_credit_ids:
                    ...
                    for payment in move.matched_credit_ids:
                        ...
                        sum += payment_cur._convert(
                            payment.amount,
                            currency_id,
                            self.env.company,
                            payment.max_date)

                    pacc += currency_id._convert(
                        move.debit - sum,
                        record.credit_currency,
                        self.env.company, move.date)
    ...
    @api.depends('sale_order_include', 'draft_invoice_include',
                 'open_invoice_include',
                 'partially_payed_invoice_include',
                 'partially_open_account_include', 'open_account_include')
    ...
    def related_moves(self):
        moves = self.env['account.move.line'].search(
            [('partner_id', '=', self.id), ('reconciled', '=', False),
             ('debit', '>', 0),
    ...
    def create(self, vals):
        ...
        if res.lco_property_product_pricelist or res.lco_property_product_pricelist_usd:
            ...
            prod_pl = self.env['product.pricelist'].browse(
                res.lco_property_product_pricelist.id)
    ...
    def change_pricelist(self):
        melleva = self.env['barmex.customer.product.pricelist'].search(
            [('partner_id.id', '=', self._origin.id)])[0]
        ...
        if self.vendor_type == 'national':
            return {
                'warning': {
                    ...
                    'message': _(
                        "Don't forget to include positive valuation for this vendor")
                }
            }
    ...
    def _update_delivery(self, lock):
        ...
        if lock:
            deliveries = self.env['stock.picking'].search(
                [('partner_id', '=', self.id), ('state', '=', 'assigned'),
    ...
    def credit_check_automated(self):
        self.env['res.partner'].search(
            [('exceed_limit', '=', False)]).credit_check()
    ...
    def _check_group(self):
        for record in self:
            ...
            purchase = self.env['res.groups'].sudo().search(
                [('name', '=', purchase)])
