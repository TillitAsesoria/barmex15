...
class Classification(models.Model):
    ...
    def get_sales(self):
        ...
        for sale in moves:
            self.sales += (
                        sale.product_uom_qty * self.product_id.standard_price)
    ...
    def get_purchases(self):
        ...
        for purchase in moves:
            self.purchases += (
                        purchase.product_uom_qty * self.product_id.standard_price)
    ...
    def update_classification(self):
        ...
        if self.can_update():
            classification = self.env['barmex.classification'].search(
                [('min_factor', '<=', self.factor),
                 ('max_factor', '>=', self.factor)], limit=1)
