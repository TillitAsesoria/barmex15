...
class BarmexAddenda(models.Model):
    ...
    sociedad = fields.Char(
        string="Número de Sociedad",
        default="1",
        required=True)
    proveedor = fields.Char(
        string="Número de Proveedor",
        default="1",
        required=True)
    correo = fields.Char(
        string="Correo",
        ...
        required=True)
    mabe_calle = fields.Char(
        string="Calle",
        default="Industrias",
        required=True)
    mabe_planta = fields.Char(
        string="Planta de Entrega",
        default="C005",
        required=True)
    mabe_ext = fields.Char(
        string="Num. Ext.",
        default="S/N",
        required=True)
    mabe_int = fields.Char(
        string="Num. Int.",
        default="S/N",
        required=True)
    por_emisor = fields.Char(
        string="Emisor",
        default="0115737",
        required=True)
    por_receptor = fields.Char(
        string="Receptor",
        default="0108918",
        required=True)
    por_planta = fields.Char(
        string="Planta Emite",
        default="R101",
        required=True)
    san_cust = fields.Char(
        string="Customer Code",
        default="43667",
        required=True)
    san_name = fields.Char(
        string="Entidad",
    ...
    env_id = fields.Char(
        ...
        default="Factura",
        required=True)
    env_trans = fields.Char(
        string="Id Transaccion",
        default="Con_Pedido",
        required=True)
    sab_tipo = fields.Char(
        string="Tipo",
        default="AddendaPCO",
        required=True)
    sab_tipoDoc = fields.Char(
        string="Tipo Documento",
        default="1",
        required=True)
    pil_proceso = fields.Char(
        string="Proceso",
        default="1",
        required=True)
    lala_seller = fields.Char(
        string="Alternate Party ID",
        default="802904",
        required=True)
