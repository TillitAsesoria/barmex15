...
class AccountPayment(models.Model):
    ...
    barmex_used_amount = fields.Monetary(
        string='Conciled Amount (Company Currency)',
        compute='_used_amount',
        currency_field='barmex_currency_id')

    barmex_used_amount_cur = fields.Monetary(
        string='Conciled Amount (Payment Currency)',
        compute='_used_amount_cur',
        currency_field='barmex_currency_id')

    barmex_payment_value = fields.Monetary(
        string='Payment Amount (Company Currency)',
        compute='_payment_value',
        currency_field='barmex_currency_id')

    barmex_unused_amount = fields.Monetary(
        string='Unconciled Amount (Company Currency)',
        compute='_unused_amount',
        currency_field='barmex_currency_id')

    barmex_unused_amount_orig = fields.Monetary(
        string='Unconciled Amount (Payment Currency)',
    ...
    def _update_collector(self):
        for record in self:
            record.update({
                'barmex_collector': self.env['barmex.collector'].search(
    ...
    def l10n_mx_edi_payment_data(self):
        ...
        date = datetime.combine(
            ...
            datetime.strptime('12:00:00', '%H:%M:%S').time()).strftime(
            '%Y-%m-%dT%H:%M:%S')
        ...
        rate = ('%.6f' % (self.currency_id.with_context(**ctx)._convert(
            1, mxn, self.company_id, self.payment_date,
            round=False))) if self.currency_id.name != 'MXN' else False
        ...
        return {
            ...
            'pay_account_ord': (
                    self.l10n_mx_edi_partner_bank_id.acc_number or '').replace(
    ...
    def action_create_payment_proposal(self):
        ...
        return {
            ...
            'view_id': self.env.ref(
                'barmex.view_account_payment_invoice_proposal_form').id,
            'context': self.env.context,
            'target': 'new',
            'type': 'ir.actions.act_window',
        }
    ...
    def create_proposal(self):
        """ Create the journal items for the payment and update the payment's state to 'draft'.
            ...
            If move_id is not empty, there will be one reconcilable move line per invoice to reconcile with.
        ...
        for rec in self:
            ...
            if any(inv.state != 'posted' for inv in rec.move_id):
            ...
            if rec.payment_type in ('inbound', 'outbound'):
                # ==== 'inbound' / 'outbound' ====
                if rec.move_id:
                    (moves[0] + rec.move_id).line_ids \
                        .filtered(
                        lambda
                            line: not line.reconciled and line.account_id == rec.destination_account_id and not (
    ...
    def action_send_notification(self):
        ...
        template_id = int(
            self.env['ir.config_parameter'].sudo().get_param(
                'account.default_vendor_notification_template'))
    ...
    def _used_amount(self):
        for record in self:
            sum = 0
            for invoice in record.move_id:
                if invoice.currency_id == record.currency_id:
                    sum += record.currency_id._convert(
                        record._get_invoice_payment_amount(invoice),
    ...
    def _used_amount_cur(self):
        for record in self:
            res = record.barmex_currency_id._convert(record.barmex_used_amount,
                                                     record.currency_id,
                                                     self.env.company,
                                                     record.payment_date)
            record.update({
    ...
    def _payment_value(self):
        for record in self:
            res = record.currency_id._convert(record.amount,
                                              record.barmex_currency_id,
                                              self.env.company,
                                              record.payment_date)
            record.update({
    ...
    def _unused_amount_value(self):
        for record in self:
            used = record.barmex_currency_id._convert(
                record.barmex_used_amount, record.currency_id,
                self.env.company,
                record.payment_date)
            record.update({
    ...
    def action_draft(self):
        ...
        if self.l10n_mx_edi_expedition_date:
            lock_date = self.l10n_mx_edi_expedition_date + relativedelta(
                days=+ 3)
        ...
        # Lock after 72 hours
            ...
            raise Warning(
                _("This payment can't be cancelled, 72 hours passed since posted"))
    ...
    def action_replace(self):
        ...
        if self.l10n_mx_edi_pac_status == 'signed' and datetime.now().date() <= lock_date:
            super(AccountPayment, self).action_draft()
            self.state = 'cancelled'
        ...
        # Lock after 72 hours
            ...
            raise Warning(
                _("This payment can't be replaced, 72 hours passed since posted"))
    ...
    def post(self):
        if self.env.company.collection_lock_date:
            if self.payment_date <= self.env.company.collection_lock_date:
    ...
    def _onchange_journal(self):
        if self.journal_id:
            ...
            if self.env.context.get('active_model') == 'account.move':
                ...
                self.amount = abs(
                    self._compute_payment_amount(invoices, self.currency_id,
                                                 self.journal_id,
                                                 self.payment_date))
