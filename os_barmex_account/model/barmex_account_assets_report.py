...
class AssetsReport(models.AbstractModel):
    ...
    def get_header(self, options):
        ...
        return [
            ...
            [
                {'name': ''},  # Description
                {'name': _('Acquisition Date'), 'class': 'text-center'},
                # Caracteristics
    ...
    def _get_lines(self, options, line_id=None):
        ...
        for al in parent_lines:
            ...
            for child in children_lines[al['asset_id']]:
                depreciation_opening += child['depreciated_start'] - child[
                    'depreciation']
            ...
            id = "_".join([self._get_account_group(al['account_code'])[0],
                           str(al['asset_id'])])
            name = str(al['asset_name'])
            line = {
                'id': id,
                'level': 1,
                'name': name if len(name) < MAX_NAME_LENGTH else name[
                                                                 :MAX_NAME_LENGTH - 2] + '...',
                'columns': [
                    ...
                    {'name': (al['asset_method'] == 'linear' and _(
                        'Linear')) or (
                                 ...
                                 'Degressive')) or _('Accelerated'),
                     'no_format_name': ''},
                    {'name': asset_depreciation_rate, 'no_format_name': ''},
                    {'name': self.env['account.asset'].browse(
                        al['asset_id']).employee_id.name,
                     'no_format_name': ''},
