...
class Product(models.Model):
    ...
    last_currency = fields.Many2one(
        'res.currency',
        string='Last purchase currency')
    last_price = fields.Monetary(
        string='Last purchase price',
        currency_field='last_currency')
    product_classification = fields.One2many(
        'barmex.product.classification',
        'product_id')

    customer_reference = fields.One2many(
    ...
    to_deliver = fields.Float(string='To deliver',
                              compute='_to_deliver')
    qty_available_barmex = fields.Float(
        string='Available',
    ...
    def classification(self):
        ...
        query = """(SELECT DISTINCT product_id FROM stock_move WHERE date::date = '{}')""".format(
            today)
            ...
            if record.type == 'product':
                ...
                for location in locations:
                    classification = self.env[
                        'barmex.product.classification'].search(
                        [('product_id', '=', record.id),
                         ('location_id', '=', location.id)], limit=1)

                    if not classification:
                        classification = self.env[
                            'barmex.product.classification'].create({
                            'product_id': record.id,
                    ...
                    classification.__class__.update_classification(
                        classification)
    ...
    def _to_deliver(self):
        ...
        deliver_id = self.env['stock.picking.type'].search(
